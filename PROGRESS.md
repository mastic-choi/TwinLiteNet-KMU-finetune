...
-3. **[진행 중 — §2.20~§2.24]** da pseudo label 개선 파이프라인.
    ...
    8. **[2026-08-12 사용자 요청, 5단계(최종 재학습) 완료 후에 할 것]** README
       ...
       - **3-way old-vs-new GIF 재제작** — **[2026-08-13 사용자 수정 지시]**
         원래 계획했던 "커브 구간 2(frame_000400~480)"는 **빼고**, 대신
         **frame_001766 부근**(§2.24에서 앙상블v2 검증 때 v1의 알려진 약점
         프레임으로 확인된 곳)에서 새로 프레임을 뽑아 그 구간 GIF를 만들 것.
         커브 구간(S자, frame_000800~935)·직진 구간(frame_000933~985)은 그대로
         유지. 비교 대상은 **원조 TwinLiteNet / v1.0.0 / v1.2.0**(사용자 표현
         그대로 — v1.0.0=지금 배포 중인 medium 40epoch 모델=§2.23~2.24에서
         "잘못 학습된 finetune 모델"로 지목했던 그 버전과 동일, v1.2.0=5단계
         최종 재학습 결과물에 붙일 예정 태그 — v1.1.0은 앙상블이라 버전 번호
         이미 씀). **[다음 세션 할 일]** 최종 재학습 끝나면: ①
         `outputs/models/best.onnx`를 GitHub 릴리즈 `v1.2.0`으로 태깅,
         ② frame_001766 앞뒤로 GIF에 쓸 연속 프레임 범위 결정(`dataset/`
         2123장 원본에서 해당 구간 찾기 — lap_005나 bootstrap_v2가 아니라
         원본 주행 시퀀스에서 찾아야 함, `curve_result.csv`/파일명으로 위치
         확인), ③ 원조/v1.0.0/v1.2.0 3-way GIF 제작(커브+새 001766구간+직진
         총 3개 GIF), ④ README GIF 섹션 교체.

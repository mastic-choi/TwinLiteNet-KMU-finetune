...
- **[사고, 2026-08-12 17:37 KST] PC가 학습 도중 예기치 않게 강제 종료됨**(Windows
  ...
  - **재시작**: seed0을 epoch15(best_da_miou=0.970, best_ll_iou=0.574 보존 확인)부터
    ...
    길어짐(~7~8시간대로 재추정).
  - **[추가 사고]** 스로틀 값을 200ms→65ms로 조정하려고 `pkill -9 -f 'python
    finetune.py'`를 실행했는데, 이 패턴이 **명령을 실행 중이던 `bash -lc "..."`
    래퍼 프로세스 자기 자신의 cmdline도 매칭해서 스크립트가 중간에 죽는
    사고**가 있었음(pkill의 `-f`는 전체 커맨드라인을 매칭하는데, 그 커맨드라인
    문자열 자체에 "python finetune.py"가 리터럴로 포함돼 있었던 게 원인).
    이 때문에 `run_ensemble_v2.sh`의 완료 검증 없는 구버전이 seed0을 또
    조용히 건너뛸 뻔했음 → 위에서 만든 **완료 검증 로직(체크포인트 epoch이
    max_epochs 도달했는지 확인 후 재시도)** 덕분에 실제로는 안전하게 복구됨.
    **교훈**: 앞으로 `pkill -f`/`pgrep -f`를 쓸 때, 패턴 문자열 자체가 현재
    실행 중인 래퍼 쉘의 커맨드라인에도 리터럴로 나타나지 않는지 주의할 것 —
    PID 기반(`ps -o pid= -C python`, `kill -9 <pid>`)이 더 안전함.
  - **[사용자 요청, 2026-08-12] GPU 사용률을 85%보다 살짝 낮은 실측값으로
    튜닝**: 200ms 스로틀 실측 61% → 65ms로 낮춤 → 실측 75~80%로 확인(사용자
    직접 GPU 모니터링 툴로 확인), **이 값 유지하기로 함**. `GPU_THROTTLE_MS=65`,
    `BATCH_SIZE=4`가 최종 채택 값.
  - **README.md 업데이트**(사용자 제공 정보 기반, 즉시 반영분): 대회 배경(제9회
    국민대 자율주행 경진대회, 자이카 Y모델, 미래관 4층 자율주행스튜디오), 차량
    PC 사양(초기 AMD Ryzen 64bit → 대회 후반 reComputer Super J4012/Jetson Orin
    NX 16GB로 본선 진행), 학습 PC 배경 정정(원래 FOSCAR 동아리 RTX 컴퓨터 지원
    예정이었으나 무산 → 최준수 개인 PC RX 9070 XT로 진행 — §2.22/§2.23의 "RTX"
    표기가 왜 나왔는지 이걸로 설명됨), "좌회전 단독 구간" 오기 정정(실제로는
    그냥 다른 지점의 커브 구간 — 파일도 `left_turn_old_vs_new.gif` →
    `curve2_old_vs_new.gif`로 개명), Special Thanks 섹션(이현기 교수님) 추가.
    **[남은 작업, §3 -3의 8번 참고]** README 나머지(3-way GIF, 라벨링 파이프라인
    재서술, 사람 vs 앙상블 비교 몽타주, 사진 전체 교체)는 5단계(최종 재학습)
    완료 후에 진행하기로 사용자가 명시적으로 미룸 — 지금 하지 말 것.

### 2.24 앙상블v2 학습 완료 + 사람 GT 재검증 완료 (2026-08-12 22:21 KST, 8회차)

- **학습 완료**: seed0~4 전부 epoch40 도달 확인(`checkpoint_epoch=40` 검증 로직
  통과). 총 소요 ~4.5시간(크래시 복구 재개 17:51 KST 시작 → 22:21 완료, 스로틀
  적용 상태 기준). 산출물(`best.pth`+`best_ll.pth` ×5, 총 20MB)을 레포에 직접
  커밋함(`outputs/ensemble_bootstrap_v2/`, `.gitignore` 예외 추가 — v1과 동일 패턴).
- **사람 GT(bootstrap_v2, 1016장 전체) 재검증** — `wsl_check_ensemble_v2_vs_gt.py`
  (신규, 이 PC WSL에서 GPU로 실행, `scripts/eval/check_ensemble_vs_human_gt.py`와
  동일 방법론이나 v2 경로+로컬 실행용으로 재작성):
  - da 6-앙상블(v2 5개 + 기존배포 best.onnx 1개, §2.23에서 사용자가 "포함 유지"로
    확정): **mean IoU=0.962**(전체 1016장) / **0.960**(held-out val 152장만 — 5개
    모델 전부 학습에 안 쓴 진짜 검증, 전체와 거의 같아서 과적합 징후 없음 확인).
    FP/GT=2.3%, FN/GT=1.7%.
  - **[중요, 방법론적 함정 발견]** 같은 GT로 v1(156장 기준 앙상블)도 재보니 오히려
    **v1이 0.968로 더 높게 나옴** — 근데 이 숫자를 그대로 믿으면 안 됨. 이유:
    이 GT의 860/1016장이 §2.20~2.22에서 **v1 6-앙상블(신규5+기존배포1) 자신이
    생성한 pseudo-label을 사람이 accept/reject/SAM보정만 한 것**(독립적으로 새로
    그린 게 아님) — 즉 v1을 v1이 절반쯤 만든 정답지에 채점하는 셈이라 자기참조적
    으로 유리함. **[교훈, ML 검증 원칙에 추가]** pseudo-label을 사람이 검수만
    하고 채택한 GT로 그 라벨을 생성한 모델 자신을 평가하면 점수가 부풀려질 수
    있음 — 특히 "trusted, 수정 없이 그대로 채택"된 프레임은 GT가 곧 그 모델의
    예측 그 자체이므로 당연히 IoU가 거의 1.0에 가까움.
  - **더 신뢰할 수 있는 검증**: §2.20에서 v1의 약점으로 명시적으로 지목됐던
    `frame_001766`/`frame_001791`(156장짜리 다양성 부족으로 오히려 악화됐던
    프레임)을 `wsl_check_specific_frames.py`로 직접 재검사 — **v1→v2로 뚜렷한
    개선 확인**: `frame_001766` IoU 0.897→**0.930**, `frame_001791` IoU
    0.901→**0.970**. 시각화(`outputs/montages/ensemble_v1_vs_v2_known_weak_frames.png`)
    로도 확인 — v1은 커브 바깥쪽 벽/테이블 영역까지 빨간색(FP) 덩어리로 침범하는
    게 뚜렷이 보이는데 v2는 거의 깨끗함. **결론: 전체 평균 IoU는 신뢰 못 하지만,
    앙상블v2를 만든 원래 목적(156장 다양성 부족 문제 해결)은 실제로 달성됨.**
  - 20장(4×5) 스팟체크 몽타주(`outputs/montages/ensemble_v2_vs_human_gt.png`)도
    제작 — 대체로 da는 초록(TP) 위주로 깨끗함(IoU 대부분 0.9대), ll은 참고용
    단일모델(seed0)이라 기존과 비슷한 수준(0.5대, 가끔 0.3대 낮은 프레임 있음 —
    가늘고 끊긴 선 구간).
...
- `~/Downloads/best.onnx`(640×360, 파라미터 ~44만, README 표 스펙과 일치)로 원조
  TwinLiteNet도 같은 PC에서 CPU 벤치마크: **12.4fps(80.5ms/frame)**, KMU medium보다
  오히려 살짝 빠름. RX 9070 XT(GPU)에서는 KMU가 원조보다 ~2배 빠른데(91.5 vs
  46.3fps) CPU에서는 역전됨 — CAAM 어텐션이 GPU 병렬화엔 유리해도 CPU 단일스레드
...
-3. **[진행 중 — §2.20~§2.24]** da pseudo label 개선 파이프라인.
    1. ~~RTX PC에서 앙상블 v2 학습~~ **완료(§2.23/§2.24)** — seed0~4 전부
       epoch40, `outputs/ensemble_bootstrap_v2/`에 커밋됨(이 PC=Windows RX
       9070 XT에서 전부 진행, Mac 경유 불필요했음 — 원래 계획한 "Mac이 조율,
       RTX가 연산" 구조 대신 이 PC 하나로 GPU 학습+검증+릴리즈까지 끝냄).
    2. ~~ENSEMBLE_DIR을 v2로 바꿔 재검증~~ **완료(§2.24)** — 전체 GT(1016장)
       기준 da IoU 0.960~0.962. **단, 이 숫자로 v1/v2 우열을 판단하면 안 됨**
       (GT의 860장이 v1 자신의 예측을 사람이 수용한 것이라 v1 쪽에 유리하게
       편향, §2.24 참고). 대신 v1의 실제 약점 프레임(frame_001766/1791)으로
       직접 검증해 v2 개선 확인(0.897→0.930, 0.901→0.970).
    3. ~~GitHub 릴리즈 + README~~ **완료** — [v1.1.0](https://github.com/mastic-choi/TwinLiteNet-KMU-finetune/releases/tag/v1.1.0)
       릴리즈, README에 "라벨링 품질 개선: 딥앙상블" 섹션 추가(배포 모델
       아님을 명시하는 톤으로).
    4. **[다음 액션]** 검증되면 이 v2
       앙상블로 **lap_005 2734장에 pseudo-label(da+ll) 생성** —
       `scripts/pseudo_label/build_pseudo_label_dataset_lap005.py`를 v2 경로
       보도록 고쳐서 **RTX(집) PC에서** 실행할 것. **[2026-08-12 사용자 결정]**
       이 스크립트가 원래 하드코딩돼 있던 `DEPLOYED_ONNX`(기존 배포 모델,
       §2.12에서 da pseudo label 오염원으로 지목됐던 그 모델)를 6-앙상블
       투표에서 **뺄지 여부를 물었는데, 사용자가 "포함 유지"로 결정함** — 앙상블
       v2 5개 + 기존 배포 모델 1개, 총 6-앙상블 소프트보팅을 §2.20 방식 그대로
       유지할 것(ENSEMBLE_DIR만 `_v1`→`_v2`로 교체, DEPLOYED_ONNX 로직은
       손대지 말 것). (Mac CPU로는 2734장에 ~70분
    ...
    8. **[완료, 2026-08-13]** README
       전면 개편:
       - **3-way old-vs-new GIF 재제작** — **[2026-08-13 사용자 수정 지시,
         재확정]** 원래 계획했던 "커브 구간 2(frame_000400~480)"는 **빼고**,
         총 **4개** GIF를 **직진 → 커브 → frame_001766 부근 → (신규) frame_
         000220~252** 순서로 배치:
         1. 직진 구간(frame_000933~985, 기존 유지)
         2. 커브 구간(S자, frame_000800~935, 기존 유지)
         3. **frame_001766 부근**(§2.24에서 앙상블v2 검증 때 v1의 알려진
            약점 프레임으로 확인된 곳) — `dataset/`에서 **frame_001650~001900
            (250장) 연속 확인 완료**(끊김 없음), 1700은 커브 체커무늬,
            1766은 사람+콘 더미 지나는 구간, 1850은 직선 복도로 이어짐 —
            이 범위 안에서 적당히 잘라 쓸 것.
         4. **[신규 추가] frame_000220~252**(33장) — `dataset/`에서 연속
            확인 완료(끊김 없음), 커브에서 직선 복도로 이어지는 구간(220번
            프레임 커브, 252번 프레임 사람 지나가는 직선 복도).
         비교 대상은 **원조 TwinLiteNet / v1.0.0 / v1.2.0**(사용자 표현 그대로
         — v1.0.0=지금 배포 중인 medium 40epoch 모델=§2.23~2.24에서 "잘못
         학습된 finetune 모델"로 지목했던 그 버전과 동일, v1.2.0=5단계 최종
         재학습 결과물에 붙일 예정 태그 — v1.1.0은 앙상블이라 버전 번호 이미
         씀). **[다음 세션 할 일]** 최종 재학습 끝나면: ① `outputs/models/
         best.onnx`를 GitHub 릴리즈 `v1.2.0`으로 태깅, ② 위 4개 구간 각각
         원조/v1.0.0/v1.2.0 3-way GIF 제작, ③ README GIF 섹션을 이 4개 순서로
         교체.
       - **[2026-08-13 사용자 지시] "라벨링 파이프라인 → 최종 모델" 몽타주
         (`outputs/montages/pipeline_montage.png`)도 새로 제작할 것** — 지금
         이미지는 옛 파이프라인(pseudo_dataset_v2, §2.9) 기준. 새 버전은 4단계
         **① 원조 TwinLiteNet 추론 → ② da 라벨(어제=2026-08-12~13 실제로 쓴
         파이프라인 그대로 — bootstrap_v2 사람검증 라벨 + 앙상블v2 6-앙상블
         pseudo-label, §2.23~2.26) → ③ YOLOPv2 기반 ll 라벨(skeleton 정제,
         기존과 동일) → ④ 이 라벨로 학습한 TwinLiteNet-KMU v1.2.0의 추론
         결과**로 교체. 같은 프레임(들)에 old→new 파이프라인 산출물을 나란히
         보여주는 기존 몽타주 스크립트 구조는 재사용하되 ②④의 소스만 최신으로
         교체하면 됨.
       - **라벨링 파이프라인 섹션 전면 재작성** — 지금 쓰는 §2.9의 옛 서술(pseudo
         label 파이프라인) 대신, 실제로 5단계 최종 모델을 만드는 데 쓴 파이프라인
         그대로 서술: 사람 라벨링(정확한 최종 장수로 — bootstrap_v2 인간검증
         134장+860장 등 실제 수치 확정 후 기입) → **딥앙상블(deep ensemble,
         영어 정식 용어 사용 + Lakshminarayanan et al. 2017 논문 링크, §2.17
         참고)** 로 928장(또는 최종 수치)의 da pseudo-label 개선 → lap_005
         2734장 추가 라벨링 → 최종 재학습. **우리가 시도했다 실패한 방법들
         (SAM, da-clipping 등)은 README에서 빼고 PROGRESS.md에만 남길 것**
         (README는 최종 성공한 경로만).
       - **신규 몽타주**: 사람이 직접 라벨링한 것과 앙상블이 자동 라벨링한 것
         비교 — **초록=사람이 칠했고 앙상블도 맞게 검출, 빨강=사람은 안 칠했는데
         앙상블이 검출(과다검출 후보), 파랑=사람은 칠했는데 앙상블이 놓침(누락)**
         색으로 오버레이, 앙상블 데이터셋에서 20장(4×5 그리드) 추출해서 제작.
       - **README의 사진/GIF를 전부 새 라벨링 파이프라인 기준으로 교체**(필요하면
         재생성) — 옛 파이프라인(앙상블 도입 전) 관련 이미지는 다 빼기.
       - **Special Thanks 섹션**(이현기 교수님)은 이미 이번 세션에 추가 완료 —
         나중에 다시 손댈 필요 없음, 참고만.
       - **[2026-08-13 완료]** 파인튜닝 결과물 이름을 **TwinLiteNet-KMU**로 확정,
         README 전반(제목/표/섹션 헤더)에 반영 완료(베이스 아키텍처를 가리키는
         "TwinLiteNetPlus" 언급은 그대로 유지 — 구분함). 데이터셋
         [Google Drive 링크](https://drive.google.com/drive/folders/1BPueQj5-elPWAJizp9um2Z4ZGoI4HCSK?usp=sharing)도
         레포 구성 섹션에 추가 완료.

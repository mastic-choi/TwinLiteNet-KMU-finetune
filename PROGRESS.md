...
-3. **[진행 중 — §2.20~§2.24]** da pseudo label 개선 파이프라인.
    ...
    8. **[2026-08-12 사용자 요청, 5단계(최종 재학습) 완료 후에 할 것]** README
       ...
       - **3-way old-vs-new GIF 재제작** — 커브 구간(S자, frame_000800~935),
         ...
         재제작(현재는 원조 vs 최신 2-way만 있음). **[확인 필요]** "잘못 학습된
         finetune 모델"이 정확히 어느 체크포인트를 가리키는지 사용자에게
         재확인할 것(letterbox 버그 있던 §2.11 이전 모델일 가능성, 또는 팀
         피드백에서 지적된 비평행 각도 문제가 있던 현재 medium 40epoch 모델일
         가능성 — 둘 다 후보, 다음 세션에서 사용자에게 명확히 물어볼 것).

...
-3. **[진행 중 — §2.20~§2.24]** da pseudo label 개선 파이프라인.
    ...
    8. **[2026-08-12 사용자 요청, 5단계(최종 재학습) 완료 후에 할 것]** README
       전면 개편:
       - **3-way old-vs-new GIF 재제작** — 커브 구간(S자, frame_000800~935),
         커브 구간 2(frame_000400~480, `curve2_old_vs_new.gif`로 개명 완료 —
         "좌회전 단독"이 아니라 이것도 커브 구간이었던 오기 정정함), 직진
         구간(frame_000933~985, 대조군) 셋 다 **원조 TwinLiteNet / "잘못
         학습된" finetune 모델 / 5단계에서 완성된 최종 finetune 모델** 3-way로
         재제작(현재는 원조 vs 최신 2-way만 있음). **[2026-08-13 사용자 확정]**
         "잘못 학습된 finetune 모델" = **지금 배포 중인 medium 40epoch 모델**
         (`outputs/models/best.onnx`, 팀 피드백에서 지적된 차선-차량 비평행
         각도 da 흔들림 한계가 있는 버전) — letterbox 버그 모델이나 오염된
         small,470 모델이 아님, 3개 후보 중 이걸로 확정.
